/****************************************************************************
 * Projeto Perceptron Multicamadas paralelo (OpenACC - MULTICORE CPU).      *
 *                                                                          *
...
typedef struct
{
  ...
  /** Vetorn que irá armazenar o erro retropropagado calculado para cada
...
typedef struct
{
  /** Vetor com a amostra (entrada da rede). */
  const float * amostra;
 ...
 * camada.
 *
 * A função será executada paralelamente na CPU, utilizando o "paralelismo
 * de neurônio", onde cada iteração (neurônio) será assignado para um núcleo.
 *
 ...
 * camada.
 *
 * A função será executada de forma sequencial na CPU.
 *
